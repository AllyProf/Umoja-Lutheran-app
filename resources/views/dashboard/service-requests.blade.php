  ...
  <div class="row mb-3">
    <div class="col-md-12">
      <div class="tile">
        <div class="tile-body">
          <form method="GET"
            action="{{ $role === 'manager' ? route('admin.service-requests') : route('reception.service-requests') }}"
            class="form-inline">
            ...
            <a href="{{ $role === 'manager' ? route('admin.service-requests') : route('reception.service-requests') }}"
                    ...
                    <tr>
                      ...
                      <td>
                        @if($request->booking_id && $request->booking)
                          <a href="{{ route($role == 'reception' ? 'reception.bookings.show' : 'admin.bookings.show', $request->booking_id) }}"
                            target="_blank">
                          ...
                          <div style="margin-top: 5px; font-size: 11px; color: #666;">
                            ...
                            @if(count($summaryData) > 0)
                              ...
                              @if(count($request->service_specific_data) > 3)
                                <button class="btn btn-xs btn-link p-0 mt-1" onclick="viewServiceDetails({{ $request->id }})"
                      ...
                      <td>{{ (float) $request->quantity }} {{ $request->service->unit }}</td>
                      <td>
                        <div>{{ number_format($request->unit_price_tsh, 0) }} TZS</div>

                      </td>
                      <td>
                        <div><strong>{{ number_format($request->total_price_tsh, 0) }} TZS</strong></div>
                      ...
                      <td>
                        <button class="btn btn-sm btn-info" onclick="viewServiceRequest({{ $request->id }})"
                          title="View Details">
                          <i class="fa fa-eye"></i>
                        </button>
                        @if($request->status === 'pending')
                          ...
                          <button class="btn btn-sm btn-danger" onclick="updateRequestStatus({{ $request->id }}, 'cancelled')"
                            title="Cancel">
                            <i class="fa fa-times"></i>
                          </button>
                        @elseif($request->status === 'approved')
                          <button class="btn btn-sm btn-primary" onclick="updateRequestStatus({{ $request->id }}, 'completed')"
        ...
        <div class="modal-body">
          <form id="updateStatusForm">
            ...
            <div class="form-group">
              ...
              <textarea class="form-control" id="reception_notes" name="reception_notes" rows="4"
                placeholder="Add any notes about this service request..."></textarea>
            </div>
