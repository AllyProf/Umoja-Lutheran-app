                  ...
                  <div class="col-md-4">
                    <div class="form-group">
                      ...
                      <input type="text" id="searchInput" class="form-control"
                        placeholder="Search by name, email, or reference..." onkeyup="filterBookings()">
                    </div>
                  </div>
                  <div class="col-md-2">
                    <div class="form-group">
              ...
              <tbody>
                @if(($bookingType ?? 'individual') == 'corporate')
                  @foreach($bookings as $group)
                    ...
                    @endphp
                    <tr class="booking-row corporate-booking-group" data-status="{{ $firstBooking->status ?? 'pending' }}"
                      data-check-in-status="{{ $firstBooking->check_in_status ?? 'pending' }}"
                      data-payment-status="{{ $firstBooking->payment_status ?? 'pending' }}"
                      data-company-id="{{ $company->id ?? '' }}" style="background-color: #f8f9fa;">
                      <td>
                      ...
                      <td>
                        <span class="badge badge-info">{{ $companyBookings->unique('room_id')->count() }}
                          room{{ $companyBookings->unique('room_id')->count() > 1 ? 's' : '' }}</span>
                      </td>
                      <td>
                        ...
                        @if($firstBooking->expires_at && $firstBooking->status == 'pending' && $firstBooking->payment_status == 'pending')
                          ...
                          @if($secondsRemaining > 0)
                            <br><small class="text-danger" id="countdown-{{ $firstBooking->id }}">
                              <i class="fa fa-clock-o"></i> Expires in: <span class="countdown-timer"
                                data-expires="{{ $expiresAt->timestamp * 1000 }}">Calculating...</span>
                            </small>
                          @else
                      ...
                      <td>
                        ...
                        @endphp
                        @php
                          $allCheckedOut = $companyBookings->every(function ($b) {
                            return ($b->check_in_status ?? 'pending') == 'checked_out';
                          });
                        @endphp
                        @if($allCheckedOut)
                        ...
                        @elseif($daysRemaining == 0)
                          ...
                          @endphp
                          <br><small class="text-warning"><i class="fa fa-exclamation-triangle"></i> Check-out today at
                            {{ $formattedTime }}</small>
                        @else
                          <br><small class="text-danger"><i class="fa fa-times-circle"></i> {{ abs($daysRemaining) }}
                            day{{ abs($daysRemaining) > 1 ? 's' : '' }} overdue</small>
                        @endif
                      </td>
                      <td>
                        {{ $totalNights }} night(s)
                      ...
                      <td>
                        @php
                          $allCompleted = $companyBookings->every(function ($b) {
                            return $b->status == 'completed' || ($b->check_in_status == 'checked_out');
                          });
                          $allConfirmed = $companyBookings->every(function ($b) {
                            return $b->status == 'confirmed' && $b->check_in_status != 'checked_out';
                          });
                          $allPending = $companyBookings->every(function ($b) {
                            return $b->status == 'pending';
                          });
                          $allCancelled = $companyBookings->every(function ($b) {
                      ...
                      <td>
                        @php
                          $allPaid = $companyBookings->every(function ($b) {
                            return $b->payment_status == 'paid';
                          });
                          $allPartial = $companyBookings->every(function ($b) {
                            return $b->payment_status == 'partial';
                          });
                          $allPendingPay = $companyBookings->every(function ($b) {
                      ...
                      <td>
                        @php
                          $allCheckedIn = $companyBookings->every(function ($b) {
                            return ($b->check_in_status ?? 'pending') == 'checked_in';
                          });
                          $allCheckedOut = $companyBookings->every(function ($b) {
                            return ($b->check_in_status ?? 'pending') == 'checked_out';
                          });
                          $allPendingCheckIn = $companyBookings->every(function ($b) {
                            return ($b->check_in_status ?? 'pending') == 'pending';
                          });
                        @endphp
                        @if($allCheckedOut)
                      ...
                      <td>
                        <div class="dropdown">
                          <button class="btn btn-sm btn-primary dropdown-toggle" type="button"
                            id="corpAction{{ $firstBooking->id }}" data-toggle="dropdown" aria-haspopup="true"
                            aria-expanded="false">
                            Action
                          </button>
                          <div class="dropdown-menu dropdown-menu-right" aria-labelledby="corpAction{{ $firstBooking->id }}">
                            <button class="dropdown-item"
                              onclick="viewCompanyBookingGroup({{ $company->id ?? 'null' }}, {{ $firstBooking->id }})">
                            ...
                            @if(in_array($firstBooking->payment_status, ['paid', 'partial']) || $firstBooking->status == 'confirmed')
                              ...
                              @if($firstPaidBooking)
                                <a class="dropdown-item"
                                  href="{{ route('payment.receipt.download', $firstPaidBooking) }}?download=1" target="_blank">
                                  <i class="fa fa-download"></i> Download Receipt
                                </a>
                              @endif
                            @endif

                            @if($firstBooking->status !== 'cancelled' && $firstBooking->status !== 'completed' && ($firstBooking->check_in_status ?? 'pending') !== 'checked_out')
                              <div class="dropdown-divider"></div>
                              <button class="dropdown-item text-danger"
                                onclick="cancelBooking({{ $firstBooking->id }}, '{{ $firstBooking->booking_reference }}')">
                    ...
                    <tr class="booking-row" data-status="{{ $booking->status }}"
                      ...
                      <td>
                        ...
                        @if($isExtended && $extendedNights > 0)
                          <br><span class="badge badge-info"
                            title="Originally scheduled to check out on {{ \Carbon\Carbon::parse($booking->original_check_out)->format('M d, Y') }}">
                            <i class="fa fa-calendar-plus-o"></i> Extended by {{ $extendedNights }}
                            night{{ $extendedNights > 1 ? 's' : '' }}
                          </span>
                        @elseif($isDecreased && $decreasedNights > 0)
                          <br><span class="badge badge-warning"
                            title="Originally scheduled to check out on {{ \Carbon\Carbon::parse($booking->original_check_out)->format('M d, Y') }}">
                            <i class="fa fa-calendar-minus-o"></i> Decreased by {{ $decreasedNights }}
                            night{{ $decreasedNights > 1 ? 's' : '' }}
                          </span>
                        @endif
                        @if($booking->extension_status === 'pending' && $booking->extension_requested_to)
                          ...
                          @if($pendingExtensionNights > 0)
                            <br><span class="badge badge-warning"
                              title="Guest has requested extension to {{ $requestedCheckOut->format('M d, Y') }}">
                              <i class="fa fa-clock-o"></i> Extension Pending (+{{ $pendingExtensionNights }}
                              night{{ $pendingExtensionNights > 1 ? 's' : '' }})
                            </span>
                      ...
                      <td>
                        ...
                        @if($booking->extension_status === 'pending' && $booking->extension_requested_to && $booking->room)
                          ...
                          @if($pendingExtensionNights > 0)
                            ...
                            <br><small class="text-muted">If approved:
                              {{ number_format($booking->total_price + $pendingExtensionCost, 0) }} TZS</small>
                          @endif
                        @endif
                      ...
                      <td>
                        ...
                        @elseif($booking->check_in_status == 'checked_in')
                          ...
                          <br><small
                            class="text-muted">{{ $booking->checked_in_at ? \Carbon\Carbon::parse($booking->checked_in_at)->format('M d, Y H:i') : '' }}</small>
                        @elseif($booking->check_in_status == 'checked_out')
                          <span class="badge badge-info">Checked Out</span>
                          <br><small
                            class="text-muted">{{ $booking->checked_out_at ? \Carbon\Carbon::parse($booking->checked_out_at)->format('M d, Y H:i') : '' }}</small>
                        @endif
                      </td>
                      <td>
                        @php
                        ...
                        <div class="dropdown">
                          <button class="btn btn-sm btn-primary dropdown-toggle" type="button"
                            id="indivAction{{ $booking->id }}" data-toggle="dropdown" aria-haspopup="true"
                            aria-expanded="false">
                            Action
                          </button>
                          <div class="dropdown-menu dropdown-menu-right" aria-labelledby="indivAction{{ $booking->id }}">
                            @if($showReminders)
                              <button class="dropdown-item" onclick="sendReminder({{ $booking->id }})">
                                <i class="fa fa-bell text-warning"></i> Send Reminders
                              </button>
                              <button class="dropdown-item" onclick="viewBooking({{ $booking->id }})">
                                <i class="fa fa-eye"></i> View More
                              </button>
                              @if(!$isReception)
                            ...
                            @elseif($booking->status == 'pending' && $booking->payment_status == 'pending')
                              <button class="dropdown-item" onclick="viewBooking({{ $booking->id }})">
                                <i class="fa fa-eye fa-fw"></i> View More
                              </button>
                              @if(!$isReception)
                                <button class="dropdown-item text-danger" onclick="deleteBooking({{ $booking->id }})">
                                  <i class="fa fa-trash fa-fw"></i> Delete
                              ...
                              <button class="dropdown-item" onclick="viewBooking({{ $booking->id }})">
                                <i class="fa fa-eye "></i> View More
                                                      ...
                                                      @if(in_array($booking->payment_status, ['paid', 'partial']) || $booking->status == 'confirmed')
                                                        <a class="dropdown-item" href="{{ route('payment.receipt.download', $booking) }}?download=1" target="_blank">
                                                          <i class="fa fa-download"></i> Download Receipt
                                                        </a>
                                                      @endif

                                              @if($booking->status == 'confirmed' && $booking->check_in_status == 'pending')
                                                <button class="dropdown-item" onclick="updateCheckIn({{ $booking->id }}, 'checked_in')">
                                                  <i class="fa fa-sign-in"></i> Check In
                                                </button>
                                              @endif

                                              @if($booking->check_in_status == 'checked_in')
                                                <button class="dropdown-item" onclick="openManagerExtensionModal({{ $booking->id }}, '{{ $booking->check_in->format('Y-m-d') }}', '{{ $booking->check_out->format('Y-m-d') }}')">
                                                  <i class="fa fa-calendar-plus-o"></i> Extend Stay
                                                </button>
                                                <button class="dropdown-item" onclick="openManagerDecreaseModal({{ $booking->id }}, '{{ $booking->check_in->format('Y-m-d') }}', '{{ $booking->check_out->format('Y-m-d') }}')">
                                                  <i class="fa fa-calendar-minus-o"></i> Decrease Stay
                                                </button>
                                              @endif

                                              <button class="dropdown-item" onclick="showNotesModal({{ $booking->id }})">
                        ...
                        <div class="mobile-booking-card booking-row corporate-booking-group"
                          ...
                          @if($company)
                            <div class="mobile-booking-info-row"
                              style="background: #f0f8ff; padding: 8px; border-radius: 4px; margin-bottom: 8px;">
                          ...
                          <div class="mobile-booking-actions">
                            <button class="btn btn-sm btn-info btn-block"
                              onclick="viewCompanyBookingGroup({{ $company->id ?? 'null' }}, {{ $firstBooking->id ?? 'null' }})"
                    ...
                    @else
                      @foreach($bookings as $booking)
                        @php
                          ...
                          $formattedTime = $hour12 . ':' . $minute . ' ' . $ampm;
                        @endphp
                        <div class="mobile-booking-card booking-row" data-status="{{ $booking->status }}"
                          data-check-in-status="{{ $booking->check_in_status ?? 'pending' }}"
                          data-payment-status="{{ $booking->payment_status ?? 'pending' }}"
                          data-booking-ref="{{ strtolower($booking->booking_reference) }}"
                          data-guest-name="{{ strtolower($booking->guest_name) }}"
                          ...
                          <div class="mobile-booking-info-row">
                            ...
                            <span class="mobile-booking-info-value">
                              ...
                              @if($daysRemaining > 0)
                                @if($weeksRemaining > 0)
                                  <br><small class="text-info">{{ $weeksRemaining }} week{{ $weeksRemaining > 1 ? 's' : '' }}
                                    remaining</small>
                                @else
                              ...
                              @else
                                <br><small class="text-danger">{{ abs($daysRemaining) }} day{{ abs($daysRemaining) > 1 ? 's' : '' }}
                                  overdue</small>
                              @endif
                            </span>
                          </div>
                          ...
                          <div class="mobile-booking-actions">
                            ...
                            @else
                              ...
                              <a href="{{ route('payment.receipt.download', $booking) }}?download=1" class="btn btn-sm btn-success"
                                target="_blank" title="Download Receipt">
                                <i class="fa fa-download"></i>
                              </a>
                              @if($booking->status == 'confirmed' && $booking->check_in_status == 'pending')
                                <button class="btn btn-sm btn-success" onclick="updateCheckIn({{ $booking->id }}, 'checked_in')"
                                  title="Check In">
                                  <i class="fa fa-sign-in"></i> Check In
                                </button>
                              @endif
            ...
            <div class="modal-body">
              <form id="managerExtensionForm">
                ...
                <div class="alert alert-info">
                  <i class="fa fa-info-circle"></i> <strong>Note:</strong> The booking will be extended and the price will be
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" id="manager_extension_reason" name="reason" rows="3"
                    placeholder="Reason for extending the booking..."></textarea>
                </div>
                <div id="managerExtensionCostPreview"
                  style="display: none; padding: 15px; background: #f8f9fa; border-radius: 5px; margin-bottom: 15px;">
                  <p class="mb-0">
            ...
            <div class="modal-body">
              <form id="managerDecreaseForm">
                ...
                <div class="alert alert-warning">
                  <i class="fa fa-info-circle"></i> <strong>Note:</strong> The booking will be decreased and the price will be
                  adjusted automatically. A refund will be calculated.
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                  ...
                  <textarea class="form-control" id="manager_decrease_reason" name="reason" rows="3"
                    placeholder="Reason for decreasing the booking..."></textarea>
                </div>
                <div id="managerDecreaseCostPreview"
                  style="display: none; padding: 15px; background: #fff3cd; border-radius: 5px; margin-bottom: 15px;">
                  <p class="mb-0">
    ...
    function cancelBooking(bookingId, bookingRef) {
      swal({
        ...
        @php
          ...
          $cancelRoute = (request()->routeIs('manager.*') || request()->routeIs('admin.*'))
