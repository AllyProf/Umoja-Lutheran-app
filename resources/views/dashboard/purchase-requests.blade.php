...
@section('content')
  ...
  @if($deadline)
    <div class="row mb-3">
      <div class="col-md-12">
        <div
          class="alert alert-info d-flex flex-column flex-md-row justify-content-between align-items-center text-center text-md-left">
          <div class="mb-2 mb-md-0">
            <i class="fa fa-info-circle"></i>
            <strong>Purchase Deadline:</strong>
            Next purchase is <strong>{{ ucfirst($deadline->day_of_week) }}</strong> at
      ...
      <ul class="nav nav-tabs nav-justified flex-column flex-sm-row" style="border-bottom: 2px solid #dee2e6;">
        <li class="nav-item">
          <a class="nav-link {{ $tab === 'new' ? 'active' : '' }}"
            href="{{ route('admin.purchase-requests.index', ['tab' => 'new']) }}"
            style="{{ $tab === 'new' ? 'background: #e07632; color: #fff; border-color: #e07632;' : 'color: #666;' }}">
            <i class="fa fa-plus-circle"></i> New Requested
            <span
              class="badge badge-{{ $tab === 'new' ? 'light' : 'secondary' }} ml-1">{{ $stats['pending'] + $stats['approved'] }}</span>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link {{ $tab === 'rejected' ? 'active' : '' }}"
            href="{{ route('admin.purchase-requests.index', ['tab' => 'rejected']) }}"
            style="{{ $tab === 'rejected' ? 'background: #dc3545; color: #fff; border-color: #dc3545;' : 'color: #666;' }}">
            <i class="fa fa-times-circle"></i> Rejected
            <span
              class="badge badge-{{ $tab === 'rejected' ? 'light' : 'secondary' }} ml-1">{{ $stats['rejected'] }}</span>
          </a>
        </li>
        <li class="nav-item">
          <a class="nav-link {{ $tab === 'completed' ? 'active' : '' }}"
            href="{{ route('admin.purchase-requests.index', ['tab' => 'completed']) }}"
            style="{{ $tab === 'completed' ? 'background: #28a745; color: #fff; border-color: #28a745;' : 'color: #666;' }}">
            <i class="fa fa-check-circle"></i> Completed Purchase
            <span
              class="badge badge-{{ $tab === 'completed' ? 'light' : 'secondary' }} ml-1">{{ $stats['purchased'] }}</span>
          </a>
        </li>
      </ul>
    </div>
      ...
      <div class="tile">
        <div class="tile-title-w-btn">
          <h3 class="title"><i class="fa fa-list"></i>
            @if($tab === 'new') New Requested Items @elseif($tab === 'rejected') Rejected Requests @else Completed
            Purchase History @endif
          </h3>
          <p>
            @if($tab === 'new')
            ...
            @foreach($departmentOrder as $dept)
              @if(isset($groupedRequests[$dept]) && $groupedRequests[$dept]->count() > 0)
                ...
                <div class="department-group mb-4"
                  style="border: 1px solid #dee2e6; border-radius: 8px; padding: 15px; background: #f8f9fa;">
                  <h4 class="mb-3"
                    style="color: {{ $departmentColors[$dept] ?? '#333' }}; border-bottom: 2px solid {{ $departmentColors[$dept] ?? '#333' }}; padding-bottom: 10px;">
                      ...
                      <table class="table table-hover table-bordered table-sm">
                        ...
                        <tbody>
                          @foreach($categoryRequests as $request)
                            <tr class="{{ $request->is_emergency ? 'table-danger' : '' }}">
                              <td>
                                @if($request->status === 'pending' || $request->status === 'approved')
                                  <input type="checkbox" class="request-checkbox" value="{{ $request->id }}"
                                    data-department="{{ $dept }}" data-status="{{ $request->status }}">
                                @endif
                              </td>
                              ...
                              <td data-label="Item Name">
                                ...
                                @if($request->is_emergency)
                                  <span class="badge badge-danger ml-1"><i class="fa fa-exclamation-triangle fast-spin"></i>
                                    Emergency</span>
                                @endif
                              </td>
                              <td data-label="Category">
                                {{ $request->category ? ucfirst(str_replace('_', ' ', $request->category)) : 'N/A' }}</td>
                              ...
                              <td data-label="Est. Cost">
                                @if($request->is_emergency && $request->estimated_cost)
                                  <span class="text-danger font-weight-bold">{{ number_format($request->estimated_cost, 0) }}</span>
                                @else
                ...
                <tbody>
                  @foreach($requests as $request)
                    <tr class="{{ $request->is_emergency ? 'table-danger' : '' }}">
                      ...
                      <td>{{ $request->category ? ucfirst(str_replace('_', ' ', $request->category)) : 'N/A' }}</td>
                      <td>{{ number_format($request->quantity, 0) }}
                        {{ $request->unit === 'bottles' ? 'PIC' : $request->unit }}</td>
                      <td>
                        @if($request->is_emergency && $request->estimated_cost)
                          <span class="text-danger font-weight-bold">{{ number_format($request->estimated_cost, 0) }}</span>
                        @else
