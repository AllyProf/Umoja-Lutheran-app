...
$variants = ProductVariant::whereHas('product', function ($q) use ($beverageCategories) {
    $q->whereIn('category', $beverageCategories);
})->where(function ($q) {
    $q->whereNull('items_per_package')->orWhere('items_per_package', '<=', 1);
})->get();

foreach ($variants as $v) {
    echo "ID: {$v->id} | Product: {$v->product->name} | Variant: {$v->variant_name} | Ratio: {$v->items_per_package}\n";
}
